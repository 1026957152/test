...
func NewAdaptor(args ...interface{}) *Adaptor {
	f := &Adaptor{
		name: hikvision.DefaultName("Firmata"),
		port: "",
		conn: nil,

		PortOpener: func(port string) (io.ReadWriteCloser, error) {
			c := &serial.Config{Name: "COM31", Baud: 57600, ReadTimeout: time.Second * 2}
			return serial.OpenPort(c) //&serial.Mode{BaudRate: 57600})
		},
		Eventer: hikvision.NewEventer(),
	}
...
// DigitalWrite writes a value to the pin. Acceptable values are 1 or 0.
func (f *Adaptor) SerialWrite(pin []byte) (err error) {
	p, err := strconv.Atoi(pin)
	if err != nil {
		return
	}
